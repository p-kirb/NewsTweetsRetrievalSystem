...
def load_data(input_path, tweets_dataframe_path):
    # Read necessary data (output of create_components.py)
    data = {}
    data['tweet_vectors'] = sp.load_npz(input_path+"documentVectors.npz")

    idf_dict = pd.read_csv(input_path+"idfDict.csv", sep=",", keep_default_na=False)
    data['idf_dict'] = dict(zip(idf_dict.term, idf_dict.idf))

    term_id = pd.read_csv(input_path+"termIndexDict.csv", sep=",", keep_default_na=False)
    ...
    factors_matrix = sp.load_npz(input_path+"factorsMatrix.npz")
    data['factors_matrix'] = factors_matrix.todok()

    cf = pd.read_csv(input_path+"cfDict.csv", sep=",", keep_default_na=False)
    data['cf'] = dict(zip(cf.term, cf.cf))
    
    parameter_matrix = sp.load_npz(input_path+"parameterMatrix.npz")
