...
# Copy setup.py from retrieval_system (handmade)
COPY ./docker_build/setup.py /usr/src/pylucene/pylucene-$PYLUCENE_VERSION/jcc
...
ENV PYTHONPATH '/retrieval_system'

# Copy data and build index
# Copy data from retrieval_system/ to /retrieval_system

COPY ./retrieval_system /retrieval_system

WORKDIR /retrieval_system

# Install requirements for lucene
RUN python -m pip install -r requirements.txt

# Run lucence script to build index
RUN python -c "import lucene; lucene.initVM().attachCurrentThread()"
RUN python -c "import nltk; nltk.download('stopwords')"
RUN python scripts/lucene_build_index.py

WORKDIR /retrieval_system/web_app
