...
def preprocess_string(text, stopwords=stopwords.words('english'), lemmatizer=WordNetLemmatizer(), check_spelling=True):
    '''
...
def remove_duplicate_tweets(df, dupliacte_col):
    """
    Remove duplicate tweets from the data.

            Parameters:
                data_path (pandas.DataFrame): Dataframe to be processed.
                dupliacte_col (str): Name of the column containing the tweet text.
                
            Returns:
                data (pandas.DataFrame): Dataframe with duplicate tweets removed.
    """

    print('\tData shape before removing duplicates: {}'.format(df.shape))
    # remove urls from tweet text so that duplicates are based on the tweet text only
    df['duplicate'] = df[dupliacte_col].apply(lambda x: remove_urls(x))
    df.drop_duplicates(subset=['duplicate'], inplace=True)
    df.drop(columns=['duplicate'], inplace=True)
    df.reset_index(drop=True, inplace=True)
    print('\tData shape after removing duplicates: {}'.format(df.shape))
