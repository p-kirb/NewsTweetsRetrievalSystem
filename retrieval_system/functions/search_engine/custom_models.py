...
def load_data(model_components_folder, tweets_dataframe_path):
    # Read necessary data (output of create_components.py)
    data = {}
    data['tweet_vectors'] = sp.load_npz(model_components_folder+"documentVectors.npz")

    idf_dict = pd.read_csv(model_components_folder+"idfDict.csv", sep=",", keep_default_na=False)
    data['idf_dict'] = dict(zip(idf_dict.term, idf_dict.idf))

    term_id = pd.read_csv(model_components_folder+"termIndexDict.csv", sep=",", keep_default_na=False)
    ...
    factors_matrix = sp.load_npz(model_components_folder+"factorsMatrix.npz")
    data['factors_matrix'] = factors_matrix.todok()

    cf = pd.read_csv(model_components_folder+"cfDict.csv", sep=",", keep_default_na=False)
    data['cf'] = dict(zip(cf.term, cf.cf))
    
    parameter_matrix = sp.load_npz(model_components_folder+"parameterMatrix.npz")
